...
def make_report(rows, basis, basis_src, txf, txf_src, selected, all_exp):
    ...
    if primary and ref:
        ...
        lines.append(f"新增壓力：{primary.get('新增Call壓力')}")
        lines.append(f"新增支撐：{primary.get('新增Put支撐')}")
    else:
    ...
    lines.append("")
    lines.append("📌 近端結算")
    lines.extend(compact_contract_lines(rows))
...
def run_analysis(prev_oi_df=None):
    ...
    summary.to_csv("txo_option_radar_summary_v21.csv", index=False, encoding="utf-8-sig")
