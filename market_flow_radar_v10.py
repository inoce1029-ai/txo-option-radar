...
def fmt_k_lots(x):
    """
    成交量以「千張」顯示。
    TWSE 成交量原始多為股數；/1000 = 張，再 /1000 = 千張。
    """
    try:
        v = float(x)
        k_lots = v / 1000000
        if abs(k_lots) >= 100:
            return f"{k_lots:.0f}千張"
        if abs(k_lots) >= 10:
            return f"{k_lots:.1f}千張"
        return f"{k_lots:.2f}千張"
    except Exception:
        return str(x)
...
def append_market_rank_section(lines, title, subdf, icon="📍"):
    ...
    lines.extend(compact_rank_lines(amt_top, ["代碼", "名稱", "成交金額", "成交量"], 10))
...
def build_report(df):
    ...
    if df.empty:
        ...
        return report, pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame()

    data_day = str(df["資料日"].iloc[0])

    # 權證獨立放最後；主要市場排行排除權證，避免干擾。
    stock_df = non_warrant_df(df)
    listed_df = market_subset(stock_df, "上市")
    otc_df = market_subset(stock_df, "上櫃")
    emerging_df = market_subset(stock_df, "興櫃")
    ...
    listed_n = int((df["市場"] == "上市").sum())
    otc_n = int((df["市場"] == "上櫃").sum())
    emerging_n = int((df["市場"] == "興櫃").sum()) if "興櫃" in df["市場"].astype(str).values else 0

    report_lines = []
    report_lines.append("📊 市場資金流雷達 v1.0")
    ...
    report_lines.append(f"權證：{warrant_count} 檔")
    report_lines.append(f"成交量集中：{market_vol_focus}")
    report_lines.append(f"成交金額集中：{market_amt_focus}")
    report_lines.append("")
    report_lines.append("━━━━━━━━━━━━━━")
    report_lines.append("")

    append_market_rank_section(report_lines, "上市一般股票 / ETF", listed_df, "🏢")
    report_lines.append("")
    report_lines.append("━━━━━━━━━━━━━━")
    report_lines.append("")
    append_market_rank_section(report_lines, "上櫃一般股票 / ETF", otc_df, "🏪")
    report_lines.append("")
    report_lines.append("━━━━━━━━━━━━━━")
    report_lines.append("")
    append_market_rank_section(report_lines, "興櫃一般股票 / ETF", emerging_df, "🌱")

    report_lines.append("")
    report_lines.append("━━━━━━━━━━━━━━")
    report_lines.append("")

    # 權證放最後；數量太少就不顯示，避免單一資料誤導。
    report_lines.append("⚠️ 權證熱點｜參考用")
    if warrant_count >= MIN_WARRANT_DISPLAY_COUNT:
        if focus.empty:
            warrant_vol_focus = "資料不足"
            warrant_amt_focus = "資料不足"
        else:
            warrant_vol_focus = "、".join(focus[focus["排行類型"].eq("成交量")]["標的顯示"].astype(str).head(5).tolist())
            warrant_amt_focus = "、".join(focus[focus["排行類型"].eq("成交金額")]["標的顯示"].astype(str).head(5).tolist())

        report_lines.append(f"今日權證：約 {warrant_count} 檔")
        report_lines.append(f"權證成交量集中：{warrant_vol_focus}")
        report_lines.append(f"權證成交金額集中：{warrant_amt_focus}")
        report_lines.append("")
        report_lines.append("📈 權證成交量 Top 10")
        report_lines.extend(compact_rank_lines(warrant_vol_top, ["代碼", "名稱", "認購認售", "成交量"], 10))
        report_lines.append("")
        report_lines.append("💰 權證成交金額 Top 10")
        report_lines.extend(compact_rank_lines(warrant_amt_top, ["代碼", "名稱", "認購認售", "成交金額", "成交量"], 10))
    else:
        report_lines.append(f"今日可辨識權證僅 {warrant_count} 檔")
        report_lines.append(f"低於 {MIN_WARRANT_DISPLAY_COUNT} 檔門檻，暫不顯示權證排行。")

    report_lines.append("")
    report_lines.append("━━━━━━━━━━━━━━")
    report_lines.append("")
    report_lines.append("📌 資料來源")
    report_lines.append(f"上市：{listed_n} 檔")
    report_lines.append(f"上櫃：{otc_n} 檔")
    report_lines.append(f"興櫃：{emerging_n} 檔")
    report_lines.append("")
    report_lines.append("說明：一般股票 / ETF 排行已排除名稱含 購 / 售 / 牛 / 熊 的權證；權證數量太少時會自動隱藏。")

    return "\n".join(report_lines), market_vol_top, market_amt_top, warrant_vol_top, warrant_amt_top, focus
...
def write_sheet(report, df, market_vol_top, market_amt_top, warrant_vol_top, warrant_amt_top, focus):
...
def main():
    ...
    report, market_vol_top, market_amt_top, warrant_vol_top, warrant_amt_top, focus = build_report(df)

    print(report)
    df.to_csv("market_flow_raw_v10.csv", index=False, encoding="utf-8-sig")
    market_vol_top.to_csv("market_volume_top10_v10.csv", index=False, encoding="utf-8-sig")
    market_amt_top.to_csv("market_amount_top10_v10.csv", index=False, encoding="utf-8-sig")

    write_sheet(report, df, market_vol_top, market_amt_top, warrant_vol_top, warrant_amt_top, focus)
