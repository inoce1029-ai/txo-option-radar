...
def wall_table(g, basis, cp):
    ...
    if basis is None or math.isnan(basis):
        x["score"] = x["oi"]
        return x.sort_values(["score", "oi"], ascending=False).head(TOP_N_WALLS)
    dist = (x["strike"] - basis).abs().clip(lower=1)
    x["score"] = x["oi"] / (1 + dist / 500)
    if cp == "C":
...
def analyze(expiry, g, basis, basis_src, txf, txf_src):
    ...
    return {
        ...
        "上方大量區": zone_from_walls(wall_table(g, basis, "C")),
        "下方大量區": zone_from_walls(wall_table(g, basis, "P")),
        "Call OI": call_oi,
        "Put OI": put_oi,
...
def make_report(rows, basis, basis_src, txf, txf_src, selected, all_exp):
    ...
    if primary and ref:
        lines.append(
            ...
            f"上方 {ref['上方大量區']} 是主要大量壓力區，下方 {ref['下方大量區']} 是主要大量支撐區。"
        )
    else:
    ...
    lines.append(fmt_line(["合約", "結算靠近價", "上方大量區", "下方大量區", "OI PCR"], widths))
    for r in rows:
        if r["合約"] == "近端綜合":
            continue
        lines.append(fmt_line([r["合約"], r["結算靠近價"], r["上方大量區"], r["下方大量區"], r["OI PCR"]], widths))
    lines.append("")
    lines.append("【大量區】")
...
def run_analysis():
    ...
    for exp in selected:
        ...
        rows.append(analyze(exp, g, basis, basis_src, txf, txf_src))

    near_df = df[df["expiry"].isin(selected)].copy()
    if not near_df.empty:
        rows.append(analyze("近端綜合", near_df, basis, basis_src, txf, txf_src))
...
def write_sheet(report, summary):
    ...
    ws2 = get_or_create_ws(sh, "近端結算表", rows=100, cols=20)
    ws2.clear()
    if summary is not None and not summary.empty:
        ws2.update("A1", [list(summary.columns)] + summary.astype(str).values.tolist())

    ws3 = get_or_create_ws(sh, "歷史紀錄", rows=1000, cols=20)
    if not ws3.get_all_values():
        ws3.append_row(["寫入時間", "資料日", "結算方向基準", "TXF參考", "近端綜合結算靠近價", "上方大量區", "下方大量區", "OI PCR", "今日總結"])
...
def main():
    summary, report = run_analysis()
    write_sheet(report, summary)
