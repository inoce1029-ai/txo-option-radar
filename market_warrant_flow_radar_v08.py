...
def build_report(df):
    ...
    report_lines = []
    report_lines.append("市場＋權證資金流雷達 v0.8")
    ...
    report_lines.extend(compact_rank_lines(market_vol_top, ["代碼", "名稱", "成交量"], 10))
    report_lines.append("")
    report_lines.append("市場成交金額 Top 10")
    report_lines.extend(compact_rank_lines(market_amt_top, ["代碼", "名稱", "成交金額", "成交量"], 10))
    report_lines.append("")
    report_lines.append(f"資料來源：上市 {int((df['市場'] == '上市').sum())} 檔；上櫃 {int((df['市場'] == '上櫃').sum())} 檔")
    report_lines.append("說明：盤後全市場含股票 / ETF / 權證；權證區塊另行篩選名稱含 購 / 售 / 牛 / 熊。")
...
def main():
    ...
    print(report)
    df.to_csv("market_warrant_flow_raw_v08.csv", index=False, encoding="utf-8-sig")
    market_vol_top.to_csv("market_volume_top10_v08.csv", index=False, encoding="utf-8-sig")
    market_amt_top.to_csv("market_amount_top10_v08.csv", index=False, encoding="utf-8-sig")
