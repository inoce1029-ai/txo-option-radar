...
def make_report(rows, basis, basis_src, txf, txf_src, selected, all_exp):
    ...
    for r in rows:
        ...
        lines.append(fmt_line([
            r["合約"], r["結算靠近價"], r["上方大量區"], r["下方大量區"], r["OI PCR"]
        ], widths))
    lines.append("")
...
def run_analysis(prev_oi_df=None):
    ...
    summary.to_csv("txo_option_radar_summary_v17.csv", index=False, encoding="utf-8-sig")
