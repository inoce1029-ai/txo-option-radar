...
OPT_INST_URL = "https://www.taifex.com.tw/data_gov/taifex_open_data.asp?data_name=CallsAndPutsDate"
FUT_INST_URL = "https://www.taifex.com.tw/data_gov/taifex_open_data.asp?data_name=FutContractsDate"
...
def analyze(expiry, g, basis, basis_src, txf, txf_src, prev_oi_df=None):
    ...
    return {
        ...
        "上方強度": wall_strength(g, basis, "C", calls),
        "下方大量區": zone_from_walls(puts),
        "下方強度": wall_strength(g, basis, "P", puts),
        "Call OI": call_oi,
        "Put OI": put_oi,
...
def main_zones(rows):
    ...
    if near:
        ...
        up_score, dn_score = near.get("上方強度"), near.get("下方強度")
    elif rows:
        up_main, dn_main = rows[0]["上方大量區"], rows[0]["下方大量區"]
        up_score, dn_score = rows[0].get("上方強度"), rows[0].get("下方強度")
    else:
        up_main = dn_main = up_score = dn_score = None
    if months:
        up_2, dn_2 = months[0]["上方大量區"], months[0]["下方大量區"]
    else:
        up_2, dn_2 = up_main, dn_main
    return up_main, up_2, dn_main, dn_2, up_score, dn_score
...
def make_report(rows, basis, basis_src, txf, txf_src, selected, all_exp, inst=None):
    now = now_taipei()
    primary = pick_primary(rows)
    near = next((r for r in rows if r["合約"] == "近端綜合"), None)
    ref = near or primary
    up_main, up_2, dn_main, dn_2, up_score, dn_score = main_zones(rows)

    lines = []
    lines.append("台指選擇權籌碼雷達 v1.5")
    ...
    if primary and ref:
        lines.append(
            ...
            f"上方 {ref['上方大量區']} 是主要大量壓力區，強度 {ref.get('上方強度')}；"
            f"下方 {ref['下方大量區']} 是主要大量支撐區，強度 {ref.get('下方強度')}。"
        )
        lines.append(
    ...
    lines.append("")
    lines.append("【近端結算重點】")
    widths = [12, 10, 14, 6, 14, 6, 8]
    lines.append(fmt_line(["合約", "結算靠近價", "上方大量區", "強度", "下方大量區", "強度", "OI PCR"], widths))
    for r in rows:
        if r["合約"] == "近端綜合":
            continue
        lines.append(fmt_line([
            r["合約"], r["結算靠近價"], r["上方大量區"], r["上方強度"],
            r["下方大量區"], r["下方強度"], r["OI PCR"]
        ], widths))
    lines.append("")
    lines.append("【大量區】")
    lines.append(f"上方主要大量區：{up_main}（強度 {up_score}）")
    lines.append(f"上方次要大量區：{up_2}")
    lines.append(f"下方主要大量區：{dn_main}（強度 {dn_score}）")
    lines.append(f"下方次要大量區：{dn_2}")
    lines.append("")
    lines.append("【共振大量區】")
    lines.extend(confluence_lines(rows))
    lines.append("")
    lines.append("【OI 變化】")
    if primary:
    ...
    lines.append("【法人方向】")
    if inst:
        lines.append(f"自營商選擇權：{inst.get('自營商選擇權')}（{inst.get('自營商說明')}）")
        lines.append(f"外資期貨：{inst.get('外資期貨')}（{inst.get('外資期貨說明')}）")
        lines.append(f"法人解讀：{inst.get('法人解讀')}")
    else:
        lines.append("法人資料不足")
    lines.append("")
    lines.append("【破防價】")
    ...
    lines.append("")
    lines.append("【程式資訊】")
    lines.append(f"目前顯示合約：{', '.join(map(str, selected))}")
    lines.append(f"全部可用合約：{', '.join(map(str, sorted(all_exp, key=sort_key)))}")
    return "\n".join(lines)
...
def find_identity_col(df):
    return find_col(df, ["身份別", "身分別", "法人"], required=False)


def find_product_col(df):
    return find_col(df, ["商品名稱", "契約名稱", "契約", "商品"], required=False)


def find_cp_col(df):
    return find_col(df, ["買賣權別", "買賣權", "權別"], required=False)


def find_first_numeric_col(df, keywords):
    col = find_col(df, keywords, required=False)
    return col


def dealer_option_direction():
    """
    自營商選擇權方向：
    優先看自營商 Call/Put 未平倉賣方口數與買方口數的差。
    粗略判斷：
    - Call 賣方偏多且大於 Put 賣方：偏壓
    - Put 賣方偏多且大於 Call 賣方：偏撐
    - 其他：中性/資料不足

    注意：自營商不等於莊家，只是選擇權賣方/避險方向參考。
    """
    try:
        raw = safe_fetch_csv(OPT_INST_URL)
        if raw.empty:
            return {"自營商選擇權": "資料不足", "自營商說明": "法人選擇權資料抓取失敗"}

        df = norm_cols(raw)
        id_col = find_identity_col(df)
        prod_col = find_product_col(df)
        cp_col = find_cp_col(df)

        if not id_col:
            return {"自營商選擇權": "資料不足", "自營商說明": "找不到身份別欄位"}

        dealer = df[df[id_col].astype(str).str.contains("自營", na=False)].copy()
        if prod_col:
            dealer = dealer[dealer[prod_col].astype(str).str.contains("臺指|台指|TXO|TX", na=False)].copy()
            if dealer.empty:
                dealer = df[df[id_col].astype(str).str.contains("自營", na=False)].copy()

        buy_oi_col = find_first_numeric_col(dealer, ["買方未平倉口數", "買方未平倉", "買方未沖銷"])
        sell_oi_col = find_first_numeric_col(dealer, ["賣方未平倉口數", "賣方未平倉", "賣方未沖銷"])
        net_oi_col = find_first_numeric_col(dealer, ["買賣淨額未平倉口數", "未平倉口數買賣淨額", "未平倉淨額"])

        if dealer.empty:
            return {"自營商選擇權": "資料不足", "自營商說明": "找不到自營商資料"}

        if cp_col and buy_oi_col and sell_oi_col:
            dealer["buy_oi"] = to_num(dealer[buy_oi_col]).fillna(0)
            dealer["sell_oi"] = to_num(dealer[sell_oi_col]).fillna(0)

            call_rows = dealer[dealer[cp_col].astype(str).str.contains("買權|Call|CALL|C", na=False)]
            put_rows = dealer[dealer[cp_col].astype(str).str.contains("賣權|Put|PUT|P", na=False)]

            call_net_sell = float(call_rows["sell_oi"].sum() - call_rows["buy_oi"].sum())
            put_net_sell = float(put_rows["sell_oi"].sum() - put_rows["buy_oi"].sum())

            if abs(call_net_sell) < 1 and abs(put_net_sell) < 1:
                return {"自營商選擇權": "中性", "自營商說明": "自營商選擇權未平倉差異不明顯"}

            if call_net_sell > put_net_sell * 1.15 and call_net_sell > 0:
                direction = "偏壓"
            elif put_net_sell > call_net_sell * 1.15 and put_net_sell > 0:
                direction = "偏撐"
            else:
                direction = "中性/雙邊賣方"

            desc = f"Call淨賣約{int(call_net_sell)}口，Put淨賣約{int(put_net_sell)}口"
            return {"自營商選擇權": direction, "自營商說明": desc}

        if net_oi_col:
            dealer["net_oi"] = to_num(dealer[net_oi_col]).fillna(0)
            net = float(dealer["net_oi"].sum())
            if net > 0:
                direction = "偏買方/中性"
            elif net < 0:
                direction = "偏賣方/中性"
                ...
                direction = "中性"
            return {"自營商選擇權": direction, "自營商說明": f"自營商選擇權淨未平倉約{int(net)}口"}

        return {"自營商選擇權": "資料不足", "自營商說明": "找不到未平倉買賣欄位"}

    except Exception as e:
        return {"自營商選擇權": "資料不足", "自營商說明": f"{type(e).__name__}: {e}"}


def foreign_futures_direction():
    """
    外資期貨方向：
    優先看外資台指期貨未平倉多空淨額。
    """
    try:
        raw = safe_fetch_csv(FUT_INST_URL)
        if raw.empty:
            return {"外資期貨": "資料不足", "外資期貨說明": "法人期貨資料抓取失敗"}
        ...
        if not id_col:
            return {"外資期貨": "資料不足", "外資期貨說明": "找不到身份別欄位"}

        foreign = df[df[id_col].astype(str).str.contains("外資|外陸", na=False)].copy()
        if prod_col:
            tx = foreign[foreign[prod_col].astype(str).str.contains("臺股期貨|台股期貨|TX|臺指|台指", na=False)].copy()
            if not tx.empty:
                foreign = tx

        net_oi_col = find_first_numeric_col(foreign, ["多空未平倉口數淨額", "未平倉口數多空淨額", "多空淨額未平倉", "未平倉淨額"])
        long_oi_col = find_first_numeric_col(foreign, ["多方未平倉口數", "多方未平倉", "多方未沖銷"])
        short_oi_col = find_first_numeric_col(foreign, ["空方未平倉口數", "空方未平倉", "空方未沖銷"])

        if foreign.empty:
            return {"外資期貨": "資料不足", "外資期貨說明": "找不到外資資料"}

        if net_oi_col:
            foreign["net"] = to_num(foreign[net_oi_col]).fillna(0)
            net = float(foreign["net"].sum())
        elif long_oi_col and short_oi_col:
            foreign["long_oi"] = to_num(foreign[long_oi_col]).fillna(0)
            foreign["short_oi"] = to_num(foreign[short_oi_col]).fillna(0)
            net = float(foreign["long_oi"].sum() - foreign["short_oi"].sum())
        else:
            return {"外資期貨": "資料不足", "外資期貨說明": "找不到期貨未平倉多空欄位"}
        ...
        return {"外資期貨": direction, "外資期貨說明": f"外資期貨淨部位約{int(net)}口"}

    except Exception as e:
        return {"外資期貨": "資料不足", "外資期貨說明": f"{type(e).__name__}: {e}"}


def institution_summary():
    dealer = dealer_option_direction()
    foreign = foreign_futures_direction()

    d = dealer.get("自營商選擇權", "資料不足")
    f = foreign.get("外資期貨", "資料不足")

    if "偏壓" in d and "偏空" in f:
        read = "上方壓力可信度提高"
    elif "偏撐" in d and "偏多" in f:
        read = "下方支撐可信度提高"
    elif ("偏壓" in d and "偏多" in f) or ("偏撐" in d and "偏空" in f):
        read = "法人方向分歧，容易區間震盪"
    elif "資料不足" in d and "資料不足" in f:
        read = "法人資料不足，暫以OI大量區為主"
    else:
        read = "法人方向中性，仍以OI大量區與共振區為主"
...
def run_analysis(prev_oi_df=None):
    ...
    summary.to_csv("txo_option_radar_summary_v15.csv", index=False, encoding="utf-8-sig")
    inst = institution_summary()
    report = make_report(rows, basis, basis_src, txf, txf_src, selected, all_exp, inst)
    print(report)
...
def write_sheet(sh, report, summary):
    ...
    if not ws3.get_all_values():
        ws3.append_row([
            ...
            "上方大量區", "上方強度", "下方大量區", "下方強度", "OI PCR", "法人解讀", "今日總結"
        ])
