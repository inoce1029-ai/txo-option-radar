...
def normalize_date_str(x):
    """Return YYYYMMDD-like string when possible."""
    s = str(x).strip().replace("/", "").replace("-", "")
    m = re.search(r"(\d{8})", s)
    return m.group(1) if m else s


def option_data_status(df):
    """Check whether TAIFEX option data appears to be updated for today's Taiwan date."""
    now = now_taipei()
    ...
    if df is None or df.empty or "date" not in df.columns:
        return {
            ...
            "data_date": "-",
            "today": today,
            "message": "沒有抓到有效 TXO 資料",
        }

    dates = [normalize_date_str(x) for x in df["date"].dropna().astype(str).unique()]
...
def parse_expiry_month(expiry):
    m = re.search(r"(\d{6})", str(expiry))
    if not m:
        return None, None
    ym = m.group(1)
...
def settlement_date_of_expiry(expiry):
    """
    用合約代碼推估實際結算日：
    - YYYYMMWn：該月第 n 個週三
    - YYYYMMFn：該月第 n 個週五
    - YYYYMM：月選，該月第 3 個週三
    """
    year, month = parse_expiry_month(expiry)
    if not year or not month:
        return None

    tag, num = parse_week_tag(expiry)
    if tag == "W":
        return nth_weekday_of_month(year, month, 2, num)  # Wednesday
    if tag == "F":
        return nth_weekday_of_month(year, month, 4, num)  # Friday

    # 月選用第 3 個週三
    return nth_weekday_of_month(year, month, 2, 3)


def is_expiry_active(expiry):
    """
    只保留尚未結算的合約。
    結算日當天 13:45 前仍保留；13:45 後視為已結算。
    若今天已經超過該結算日，也直接略過。
    """
    sd = settlement_date_of_expiry(expiry)
    if sd is None:
        return True
    ...
    if sd < today:
        return False
    if sd == today and (now.hour, now.minute) >= (SETTLE_HOUR, SETTLE_MINUTE):
        return False
    return True
...
def sort_key(expiry):
    """
    排序改為實際結算日優先。
    這樣 202605W1 若已在 5/6 結算，5/7 不會再當今日總結主合約。
    """
    e = str(expiry).upper().strip()
    sd = settlement_date_of_expiry(e)

    tag, num = parse_week_tag(e)
    tag_order = 1 if tag == "W" else 2 if tag == "F" else 3

    if sd is not None:
        return (sd, tag_order, e)

    # fallback
    m = re.search(r"(\d{6})", e)
    month_num = int(m.group(1)) if m else 999999
    return (datetime(2099, 12, 31).date(), tag_order, month_num, e)
...
def filter_expiries(expiries):
    ...
    - 月選當月與次月
    """
    active = [exp for exp in expiries if is_expiry_active(exp)]
...
def pick_primary(rows):
    ...
    for r in rows:
        ...
        if exp == "近端綜合":
            continue
        if not is_expiry_active(exp):
            continue
        sd = settlement_date_of_expiry(exp)
        tag, _ = parse_week_tag(exp)
        tag_order = 1 if tag in ["W", "F"] else 2
        candidates.append((sd if sd is not None else datetime(2099, 12, 31).date(), tag_order, str(exp), r))
...
def compact_contract_lines(rows):
    lines = []
    for r in rows:
        if r["合約"] == "近端綜合":
            continue
        lines.append(f"{r['合約']}｜靠 {r['結算靠近價']}｜壓 {r['上方大量區']}｜撐 {r['下方大量區']}")
    return lines if lines else ["資料不足"]


def compact_direction_lines(rows, basis):
    lines = []
    for r in rows:
        if r["合約"] == "近端綜合":
            continue
        lines.append(f"{r['合約']}：{r['結算靠近價']}｜{short_dir_text(direction(basis, r['結算靠近價']))}")
    return lines if lines else ["資料不足"]


def make_report(rows, basis, basis_src, txf, txf_src, selected, all_exp, data_status=None):
    now = now_taipei()
    ...
    lines = []
    lines.append("📊 台指選擇權籌碼雷達 v2.2")
    if data_status:
        lines.append(f"{data_status.get('emoji', '⚪')} 資料狀態：{data_status.get('status', '未知')}")
        lines.append(f"📅 資料日：{data_status.get('data_date', now.strftime('%Y%m%d'))}")
    else:
        lines.append(f"📅 資料日：{now.strftime('%Y/%m/%d')}")
    lines.append(f"🕒 最後更新：{now.strftime('%Y/%m/%d %H:%M')}")
    lines.append(f"🎯 基準：加權 {round(basis) if basis else None}")
    lines.append(f"📍 期貨參考：TXF {round(txf) if txf else None}")
    lines.append("")
    lines.append("🔥 今日總結")
    if primary and ref:
        lines.append(f"主合約：{primary['合約']}")
        lines.append(f"結算靠近：{primary['結算靠近價']}（{short_dir_text(direction(basis, primary['結算靠近價']))}）")
        lines.append(f"上方壓力：{ref['上方大量區']}")
        lines.append(f"下方支撐：{ref['下方大量區']}")
    else:
        lines.append("資料不足，暫不判斷。")
    ...
    lines.append(f"Buy Call：{flow['可能BuyCall區']}")
    lines.append(f"Buy Put：{flow['可能BuyPut區']}")
    lines.append(f"Sell Call 區：{flow['主要SellCall區']}")
    lines.append(f"Sell Put 區：{flow['主要SellPut區']}")

    lines.append("")
    lines.append("━━━━━━━━━━━━━━")
    lines.append("")
    lines.append("🧱 大量區")
    lines.append(f"主壓力：{up_main}")
    lines.append(f"次壓力：{up_2}")
    lines.append(f"主支撐：{dn_main}")
    lines.append(f"次支撐：{dn_2}")
    ...
    lines.append("📈 OI 變化")
    if primary:
        lines.append(f"{primary['合約']} 新增Call：{primary.get('新增Call壓力')}")
        lines.append(f"{primary['合約']} 新增Put：{primary.get('新增Put支撐')}")
    ...
    if primary:
        ...
        lines.append(f"站上 {up_break}：上方壓力可能失效")
        lines.append(f"跌破 {dn_break}：下方支撐可能失效")
    else:
        lines.append("資料不足")
    ...
    lines.append("")
    lines.append("🧲 結算靠近方向")
    lines.extend(compact_direction_lines(rows, basis))
...
def run_analysis(prev_oi_df=None):
    ...
    data_status = option_data_status(df)
    if df.empty:
        report = "❌ 資料狀態：抓取失敗\n沒有抓到 TXO 台指選擇權資料。請檢查期交所資料是否已更新。"
        print(report)
    ...
    summary.to_csv("txo_option_radar_summary_v22.csv", index=False, encoding="utf-8-sig")
    report = make_report(rows, basis, basis_src, txf, txf_src, selected, all_exp, data_status)
    print(report)
...
def write_sheet(sh, report, summary):
    ...
    for line in report.splitlines():
        if line.startswith("主合約：") or line.startswith("結算靠近："):
            summary_line = line
            break
