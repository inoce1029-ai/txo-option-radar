...
def build_intraday_section(df):
    ...
    lines.append("")
    lines.append("成交量急增 Top 10")
    lines.append(fmt_table(vol_top, ["代碼", "名稱", "標的", "認購認售", "盤中價", "漲跌幅%", "盤中成交量"]))
    lines.append("")
    lines.append("成交金額急增 Top 10")
    lines.append(fmt_table(amt_top, ["代碼", "名稱", "標的", "認購認售", "盤中價", "估算成交金額", "盤中成交量"]))
...
def build_report(df):
    ...
    report_lines = []
    report_lines.append("市場＋權證資金流雷達 v0.5")
    report_lines.append(f"資料日：{data_day}")
    report_lines.append(f"最後更新時間：{now.strftime('%Y/%m/%d %H:%M')}")
    report_lines.append("")
    report_lines.append(intraday_text)
    report_lines.append("")
    report_lines.append("【每日盤後資料更新】")
    report_lines.append(f"更新時間：{now.strftime('%Y/%m/%d %H:%M')}")
    report_lines.append("")
    report_lines.append("【今日總結】")
    report_lines.append(
        f"今日共抓到 {len(df)} 檔市場資料，其中權證約 {warrant_count} 檔。"
        f"全市場成交量集中：{market_vol_focus}。"
        f"全市場成交金額集中：{market_amt_focus}。"
    )
    report_lines.append("")
    report_lines.append("【市場成交量 Top 10】")
    report_lines.append(fmt_table(market_vol_top, ["代碼", "名稱", "標的", "認購認售", "成交量", "收盤價", "漲跌"]))
    report_lines.append("")
    report_lines.append("【市場成交金額 Top 10】")
    report_lines.append(fmt_table(market_amt_top, ["代碼", "名稱", "標的", "認購認售", "成交金額", "成交量", "收盤價"]))
    report_lines.append("")
    report_lines.append("【權證成交量 Top 10】")
    if warrant_vol_top.empty:
        report_lines.append("今日權證成交量資料不足。")
    else:
        report_lines.append(fmt_table(warrant_vol_top, ["代碼", "名稱", "標的", "認購認售", "成交量", "收盤價", "漲跌"]))
    report_lines.append("")
    report_lines.append("【權證成交金額 Top 10】")
    if warrant_amt_top.empty:
        report_lines.append("今日權證成交金額資料不足。")
    else:
        report_lines.append(fmt_table(warrant_amt_top, ["代碼", "名稱", "標的", "認購認售", "成交金額", "成交量", "收盤價"]))
    report_lines.append("")
    report_lines.append("【權證標的集中】")
    if focus.empty:
...
def main():
    ...
    print(report)
    df.to_csv("market_warrant_flow_raw_v05.csv", index=False, encoding="utf-8-sig")
    market_vol_top.to_csv("market_volume_top10_v05.csv", index=False, encoding="utf-8-sig")
    market_amt_top.to_csv("market_amount_top10_v05.csv", index=False, encoding="utf-8-sig")
