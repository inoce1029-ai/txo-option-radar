...
def is_warrant_name(name):
    """
    只保留真正權證名稱。
    避免 TWSE ALLBUT0999 表格因標題含「權證」而把普通股票也納入。
    """
    s = str(name)
...
def build_report(df):
    ...
    report_lines = []
    report_lines.append("權證資金流雷達 v0.2")
...
def main():
    ...
    print(report)
    df.to_csv("warrant_flow_raw_v02.csv", index=False, encoding="utf-8-sig")
    vol_top.to_csv("warrant_volume_top10_v02.csv", index=False, encoding="utf-8-sig")
    amt_top.to_csv("warrant_amount_top10_v02.csv", index=False, encoding="utf-8-sig")
