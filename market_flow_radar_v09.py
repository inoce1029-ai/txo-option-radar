...
def append_market_rank_section(lines, title, subdf):
    lines.append(title)
...
def build_report(df):
    ...
    report_lines = []
    report_lines.append("市場資金流雷達 v0.9")
    report_lines.append(f"資料日：{data_day}")
    report_lines.append(f"最後更新時間：{now.strftime('%Y/%m/%d %H:%M')}")
    report_lines.append("")
    report_lines.append("【每日盤後市場熱點】")
    report_lines.append(f"更新時間：{now.strftime('%Y/%m/%d %H:%M')}")
    report_lines.append(f"今日共抓到 {len(df)} 檔市場資料；一般股票 / ETF 約 {len(stock_df)} 檔；權證約 {warrant_count} 檔。")
    ...
    if warrant_count >= MIN_WARRANT_DISPLAY_COUNT:
        ...
        report_lines.append("")
        report_lines.append("【權證熱點｜參考用】")
        report_lines.append(f"今日可辨識權證僅 {warrant_count} 檔，低於 {MIN_WARRANT_DISPLAY_COUNT} 檔門檻，暫不顯示權證排行。")

    report_lines.append("")
    listed_n = int((df["市場"] == "上市").sum())
    otc_n = int((df["市場"] == "上櫃").sum())
    emerging_n = int((df["市場"] == "興櫃").sum()) if "興櫃" in df["市場"].astype(str).values else 0
    report_lines.append(f"資料來源：上市 {listed_n} 檔；上櫃 {otc_n} 檔；興櫃 {emerging_n} 檔")
...
def main():
    ...
    print(report)
    df.to_csv("market_flow_raw_v09.csv", index=False, encoding="utf-8-sig")
    market_vol_top.to_csv("market_volume_top10_v09.csv", index=False, encoding="utf-8-sig")
    market_amt_top.to_csv("market_amount_top10_v09.csv", index=False, encoding="utf-8-sig")
