        ...
        try:
            ...
            for t in tables:
                ...
                for row in data:
                    ...
                    # 權證通常名稱包含「購/售」，或該表標題包含權證
                    if "權證" not in title and ("購" not in name and "售" not in name):
                        continue
...
def build_report(df):
    ...
    if df.empty:
        report = "\n".join([
            ...
            "",
            "【今日總結】",
            "今日未抓到權證資料，請檢查 TWSE / TPEx 資料來源。",
        ])
        return report, pd.DataFrame(), pd.DataFrame(), pd.DataFrame()

    data_day = str(df["資料日"].iloc[0])
    vol_top = df.sort_values("成交量", ascending=False).head(TOP_N)
    amt_top = df.sort_values("成交金額", ascending=False).head(TOP_N)
    ...
    for metric in ["成交量", "成交金額"]:
        ...
        x = x[x["標的"] != "-"]
        if not x.empty:
            g = x.groupby(["標的", "認購認售"], as_index=False)[metric].sum()
    ...
    report_lines = []
    report_lines.append("權證資金流雷達 v0.1")
    report_lines.append(f"資料日：{data_day}")
    report_lines.append(f"最後更新時間：{now.strftime('%Y/%m/%d %H:%M')}")
    report_lines.append("")
    report_lines.append("【今日總結】")
    report_lines.append(summarize(df, vol_top, amt_top))
    report_lines.append("")
    report_lines.append("【成交量 Top 10】")
    report_lines.append(fmt_table(vol_top, ["代碼", "名稱", "標的", "認購認售", "成交量", "收盤價", "漲跌"]))
    report_lines.append("")
    report_lines.append("【成交金額 Top 10】")
    report_lines.append(fmt_table(amt_top, ["代碼", "名稱", "標的", "認購認售", "成交金額", "成交量", "收盤價"]))
    report_lines.append("")
    report_lines.append("【標的集中】")
    report_lines.append(f"成交量集中：{top_underlyings(vol_top, '成交量')}")
    report_lines.append(f"成交金額集中：{top_underlyings(amt_top, '成交金額')}")
    report_lines.append(f"資料來源：上市 {int((df['市場'] == '上市').sum())} 檔；上櫃 {int((df['市場'] == '上櫃').sum())} 檔")

    return "\n".join(report_lines), vol_top, amt_top, focus
...
def write_sheet(report, df, vol_top, amt_top, focus):
    ...
    ws_report.clear()
    ws_report.update("A1", [[line] for line in report.splitlines()])

    write_df(get_or_create_ws(sh, "權證成交量Top10", rows=100, cols=20), vol_top)
    write_df(get_or_create_ws(sh, "權證成交金額Top10", rows=100, cols=20), amt_top)
    write_df(get_or_create_ws(sh, "權證標的集中", rows=100, cols=20), focus)
    write_df(get_or_create_ws(sh, "權證原始資料", rows=5000, cols=20), df)

    # 歷史簡表
    ws_hist = get_or_create_ws(sh, "權證歷史紀錄", rows=1000, cols=20)
    ...
    ws_hist.append_row([
        ...
        data_day,
        top_underlyings(vol_top, "成交量") if vol_top is not None and not vol_top.empty else "-",
        top_underlyings(amt_top, "成交金額") if amt_top is not None and not amt_top.empty else "-",
        summary_line,
    ])
...
def main():
    ...
    report, vol_top, amt_top, focus = build_report(df)

    print(report)
    df.to_csv("warrant_flow_raw.csv", index=False, encoding="utf-8-sig")
    vol_top.to_csv("warrant_volume_top10.csv", index=False, encoding="utf-8-sig")
    amt_top.to_csv("warrant_amount_top10.csv", index=False, encoding="utf-8-sig")

    write_sheet(report, df, vol_top, amt_top, focus)
