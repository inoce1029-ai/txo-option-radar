...
def parse_foreign_table(obj, market_name, data_day):
    ...
    for t in tables:
        ...
        code_f = find_field(fields, ["證券代號", "代號"], required=False)
        name_f = find_field(fields, ["證券名稱", "名稱"], required=False)
...
def foreign_rank_lines(df, ascending=False):
    ...
    x = df.copy()
    if ascending:
        x = x[x["外資買賣超股數"] < 0].sort_values("外資買賣超股數", ascending=True).head(TOP_N)
    else:
        x = x[x["外資買賣超股數"] > 0].sort_values("外資買賣超股數", ascending=False).head(TOP_N)
    if x.empty:
        return ["資料不足"]
    lines = []
    for i, (_, r) in enumerate(x.iterrows(), start=1):
        lines.append(f"{i}. {r.get('代碼')}｜{r.get('名稱')}｜{fmt_foreign_wan_lots(r.get('外資買賣超股數'))}")
    return lines
...
def build_report(df):
    ...
    report_lines = []
    report_lines.append("📊 市場資金流雷達 v1.2")
    ...
    foreign_buy_top = (foreign_df[foreign_df["外資買賣超股數"] > 0]
                       .sort_values("外資買賣超股數", ascending=False).head(TOP_N)) if not foreign_df.empty else pd.DataFrame()
    foreign_sell_top = (foreign_df[foreign_df["外資買賣超股數"] < 0]
                        .sort_values("外資買賣超股數", ascending=True).head(TOP_N)) if not foreign_df.empty else pd.DataFrame()
...
def write_sheet(report, df, market_vol_top, market_amt_top, warrant_vol_top, warrant_amt_top, focus, foreign_buy_top, foreign_sell_top):
    ...
    write_df(get_or_create_ws(sh, "外資買超Top10", rows=100, cols=20), foreign_buy_top)
    write_df(get_or_create_ws(sh, "外資賣超Top10", rows=100, cols=20), foreign_sell_top)
...
def main():
    ...
    print(report)
    df.to_csv("market_flow_raw_v12.csv", index=False, encoding="utf-8-sig")
    market_vol_top.to_csv("market_volume_top10_v12.csv", index=False, encoding="utf-8-sig")
    market_amt_top.to_csv("market_amount_top10_v12.csv", index=False, encoding="utf-8-sig")
    foreign_buy_top.to_csv("foreign_buy_top10_v12.csv", index=False, encoding="utf-8-sig")
    foreign_sell_top.to_csv("foreign_sell_top10_v12.csv", index=False, encoding="utf-8-sig")
